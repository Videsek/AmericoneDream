...
void DrawImage( GLuint sourceTexture, Color& color, float xPos, float yPos, float width, float height,float radRotation)
{
	...
	float cosRot = cosf(radRotation);
	float sinRot = sinf(radRotation);
	rotateOffset.x = (cosRot * halfW) - (sinRot * halfH);
	rotateOffset.y = (sinRot * halfW) + (cosRot * halfH);


	// Top-left vertex (corner)
	glTexCoord2i( 0, 0 );
	glVertex3f( xPos-rotateOffset.x, yPos-rotateOffset.y, 0 );	

	// Top-right vertex (corner)
	glTexCoord2i( 1, 0 );
	glVertex3f( xPos+rotateOffset.y, yPos-rotateOffset.x, 0 );	
